...
- The `LogLine` enum covers both core conversation records and newer metadata/event records, including user/assistant
  ...
  #N]`placeholders in its text and the`image`content blocks that carry the data; logged as`null`when nothing was pasted, and only ever`null`
  on assistant turns, but emitted on both roles so both structs need the field) (all added in Claude Code 2.1.257+)
